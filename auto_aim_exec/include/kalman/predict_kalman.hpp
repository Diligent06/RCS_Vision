...
private:
    Eigen::Matrix3d R_CI = Eigen::Matrix3d::Zero(3, 3);           // 陀螺仪坐标系到相机坐标系旋转矩阵EIGEN-Matrix
    Eigen::Matrix3d F = Eigen::Matrix3d::Zero(3, 3);              // 相机内参矩阵EIGEN-Matrix
    Eigen::MatrixXd C;  // 相机畸变矩阵EIGEN-Matrix
...
public:
    PredictorKalman();
    Eigen::Vector3d predict(Eigen::Vector3d &_pc, float *_q);
    
    //bool predict(Detection_pack &, RobotCmd &, cv::Mat &);
};
extern PredictorKalman predictkalman;
